...
forecastSV <- function(y, theta, H = 10, S = 50){
  vFC <- vFC <- theta[1] * theta[3] + rnorm(1, 0, sqrt(theta[2]))
  for(t in 2:H){
...
symKLVB <- function(lambda, draws, post, maxIter = 1000, threshold = 0.001, alpha = 0.01){
  ...
  while(diff > threshold & iter <= maxIter){
    ...
    w <- qDens / post
    KL[iter] <- mean(w * (log(qDens / post)) + log(post / qDens))
    
    gradient <- apply(score, 1, function(x) mean(x * (w * log(qDens / post) - 1)))
    ...
    if(iter %% 5 == 0){
      oldMeanKL <- meanKL
      meanKL <- mean(KL[iter:(iter- 4)])
      diff <- abs(meanKL - oldMeanKL)
    }
    #if(iter %% 100 == 0){
  ...
  list(lambda = lambda, iter = iter-1, KL = KL[1:(iter-1)])
}
...
SMC <- function(y, lambda, draws, S = 200, suppressProgress = FALSE){
  draws <- as.matrix(draws)[,1:2]
  iter <- nrow(draws)
  w <- rep(0, iter)
  x <- rep(0, iter)
  ...
  w <- w - max(w)
  w <- exp(w) / sum(exp(w))
  resample <- sample(1:nrow(draws), prob = w)

  cbind(draws[resample, ], x[resample])
}
...
for(t in 2:length(Tseq)){
  ...
  if(t == 2){
    ...
    smc <- as.matrix(smc)
    smcTime <- Sys.time() - startTime
    if(attr(smcTime, 'units') == 'mins'){
      smcTime <- as.numeric(smcTime)
      smcTime <- smcTime * 60
    } else {
      smcTime <- as.numeric(smcTime)
    }
    ...
    margins <- apply(smc, 2, function(x){
      dens <- density(x)
      approxfun(dens)
    })
    cdf <- apply(smc, 2, function(x){
    ...
    # Fit VB
    lambda <- c(colMeans(smc), chol(cov(smc)))
    
    vb <- symKLVB(lambda, draws, postDens, alpha = 0.001, threshold = 0.0005)
  ...
  for(i in 1:N){
    fcSims[i, ] <- forecastSV(y[1:Tseq[t]], smc[subset[i], ])
  }
  densFuns <- apply(fcSims, 2, function(x){
    xSub <- x[x > quantile(x, 0.01) & x < quantile(x, 0.999)]
    dens <- density(xSub)
    approxfun(dens)
  })
...
write.csv(results, paste0('VMCMC/rep', id + 200, '.csv'), row.names = FALSE)